...
- `V4.2.1`：在 `level2` 上执行 `baseline + pH + alkalinity + missing flags`
- `V4.2.2`：在 `level2` 中仅保留 `pH` 与 `alkalinity` 同时非缺失的完整子集，重复对应稳健性实验
...
- `V4.2.1` 固定使用 `level2`
- `V4.2.2` 仍以 `level2` 为母集，但仅保留 `ph_sample_weighted_mean` 与 `alkalinity_sample_weighted_mean_mg_l` 同时非缺失的记录

`level2` 总样本数为 `26,975`。其中：

- `pH` 缺失行数：`10,047`
- `alkalinity` 缺失行数：`1,529`
- `pH / alkalinity` 任一缺失行数：`11,503`
- `pH + alkalinity` 完整子集行数：`15,472`
- 完整子集占 `level2` 比例：`57.36%`
...
- `V4.2.1`：保留 `level2` 样本，不做 complete-case；数值变量走中位数填补，保留 `pH` 与 `alkalinity` 的 missing flag
...
| --- | --- | ---: | ---: | ---: | ---: |
| `V4.1 baseline` | `level1 + baseline_default` | 0.0696 | 0.7218 | 0.0690 | 0.7046 |
| `level2 baseline reference` | `level2 + baseline_default` | 0.0652 | 0.7212 | 0.1039 | 0.7395 |
| `V4.2.1` | `level2 + mechanistic_stage1` | 0.0775 | 0.7586 | 0.1941 | 0.7986 |
| `level2 complete-case baseline reference` | `level2 complete-case + baseline_default` | 0.0815 | 0.7213 | 0.1115 | 0.7452 |
| `V4.2.2` | `level2 complete-case + mechanistic_stage1` | 0.1298 | 0.7902 | 0.1781 | 0.8221 |
| `V4.2.2b` | `level2 complete-case + mechanistic_stage1 without missing flags` | 0.1298 | 0.7902 | 0.1781 | 0.8221 |

关键信息：

- `V4.2.1` 相对 `level2 baseline reference` 的测试集增益为：`PR-AUC +0.0902`、`ROC-AUC +0.0591`
- `V4.2.2` 相对 `level2 complete-case baseline reference` 的测试集增益为：`PR-AUC +0.0666`、`ROC-AUC +0.0769`
...
| 版本 | 样本说明 | Validation PR-AUC | Validation ROC-AUC | Test PR-AUC | Test ROC-AUC |
| --- | --- | ---: | ---: | ---: | ---: |
| `V4.1 baseline` | `level1 + baseline_default` | 0.1216 | 0.7642 | 0.1162 | 0.7452 |
| `level2 baseline reference` | `level2 + baseline_default` | 0.1208 | 0.7681 | 0.1858 | 0.7816 |
| `V4.2.1` | `level2 + mechanistic_stage1` | 0.1416 | 0.8112 | 0.3061 | 0.8412 |
| `level2 complete-case baseline reference` | `level2 complete-case + baseline_default` | 0.0950 | 0.7362 | 0.1854 | 0.7744 |
| `V4.2.2` | `level2 complete-case + mechanistic_stage1` | 0.1660 | 0.8275 | 0.2830 | 0.8716 |
| `V4.2.2b` | `level2 complete-case + mechanistic_stage1 without missing flags` | 0.1660 | 0.8275 | 0.2830 | 0.8716 |

关键信息：

- `V4.2.1` 相对 `level2 baseline reference` 的测试集增益为：`PR-AUC +0.1203`、`ROC-AUC +0.0596`
- `V4.2.2` 相对 `level2 complete-case baseline reference` 的测试集增益为：`PR-AUC +0.0975`、`ROC-AUC +0.0972`
...
- `V4.1 baseline`
- 对应的 `level2 baseline reference`

这说明把 `pH + alkalinity + missing flags` 纳入 `level2` 后，提升不是单一指标上的偶然波动，而是 `PR-AUC` 与 `ROC-AUC` 都出现了同向改善。
...
- `anchored` 的 `V4.2.1` 相对 `level2 baseline reference` 提升到 `PR-AUC 0.3061`
- `regulatory` 的 `V4.2.1` 相对 `level2 baseline reference` 提升到 `PR-AUC 0.1941`
...
更稳妥的表述是：本轮结果说明 `pH` 与总碱度在 `level2` 样本内具有可复现的预测增益，但仍属于预测层面的机制增强证据，而不是因果机制结论。
...
- `scripts/v4_tthm_training_common.py` 已扩展为支持任意特征组、`level2` 样本和 complete-case 对照
