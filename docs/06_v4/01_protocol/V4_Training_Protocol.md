...
`第一级样本` 是当前第一版全国主模型的默认主线层级。

### 7.2 `第二级样本`

- 筛选条件：`level2_flag = 1`
- 定义：在 `第一级样本` 基础上，`n_core_vars_available >= 2`
- 当前数量：`26,975`
- 定位：增强模型与机制变量增益测试样本
...
- 筛选条件：`level3_flag = 1`
- 定义：在 `第一级样本` 基础上，`n_core_vars_available >= 3`
- 当前数量：`6,193`
- 定位：高信息稳健性检查样本
...
- 主结果默认来自 `第一级样本`
- 增强实验优先在 `第二级样本` 上进行
- `第三级样本` 优先用于稳健性、解释性与敏感性检查

当前禁止：

- 直接拿 `第二级样本` 或 `第三级样本` 结果替代 `第一级样本` 全国主线结论
...
- 第一轮正式实验：`第一级样本 + baseline_default`
- 第二轮条件实验：`第一级样本 + baseline_default + conditional`
- 第三轮增强实验：`第二级样本 + baseline_default + enhanced_default`
...
3. `第二级样本` 或 `第三级样本` 的增强结果不能直接替代 `第一级样本` 全国主线结论
