...
3. `第一级样本` 用于全国主线 baseline
4. `第二级样本` 用于增强变量主实验
5. `第三级样本` 用于高信息、小样本、稳健性与专题实验
...
因为 `pH` 和 `alkalinity` 在 `第二级样本` 中仍可能存在缺失。  
当前默认不是只保留完整样本，而是：
...
- 只保留 `pH` 和 `alkalinity` 同时非缺失的 `第二级样本` 子集
...
- 只保留 `pH`、`alkalinity`、`TOC` 同时非缺失的 `第二级样本` 子集
...
- 只保留 `pH`、`alkalinity`、`TOC`、`free_chlorine` 同时非缺失的 `第二级样本` 子集
