...
2. `V4.3` 已确认：`TOC` 是当前第三层主线上最可信的机制增强变量，但其主要证据来自 `第二级样本` 机制增强链。
3. `V4.4` 已确认：`free_chlorine` 在 full `第二级样本` 上只有弱边际增益，且数值信号与缺失模式信号无法稳健拆分。
4. `V4.4b` 已确认：`total_chlorine` 当前不具备继续进入第三层正式增量实验的条件。
5. `V4.5` 已确认：结构/覆盖条件特征在 `第一级样本` 全国主线上对 `regulatory` 与 `anchored` 两条任务都带来明显且稳定的预测提升，且主体增益来自覆盖计数与结构条件特征本身，而不是单个 `annual_match_quality_tier`。
...
### 3.4 `第一级样本` 与 `第二级样本` 的角色继续区分

- 主实验仍优先围绕 `第一级样本` 全国主线展开
- 如有必要，可补 `第二级样本` 对照，但不得重新让 `第二级样本` 替代全国主线结论
...
`V4.6` 的核心风险不是跑不出结果，而是误把 treatment summary 的工程背景代理信号写成机制发现，或误把其增益与 `V4.5` 的结构/覆盖代理信号混在一起。因此本轮至少必须保留以下 4 组 `第一级样本` 版本：
...
### 6.2 `第一级样本 structural conditional reference`
...
### 6.3 `第一级样本 treatment summary increment`
...
### 6.4 `第一级样本 structural + treatment combined`
...
如资源允许，建议补一个 `第二级样本 structural + treatment combined reference`：

- 样本：全部 `第二级样本`
- 特征：与 `第一级样本 structural + treatment combined` 相同
...
1. 在 `第一级样本 baseline` 基础上加入 treatment summary 特征后，是否带来稳定提升？
...
5. `第一级样本` 与 `第二级样本` 对 treatment summary 的响应方向是否一致？
...
5. 跳过 `第一级样本 structural conditional reference`，直接只比较 `baseline + treatment`
...
- 在第三层 `PWS-year` 全国主线 `第一级样本` 上测试 treatment summary 特征的独立增量价值
